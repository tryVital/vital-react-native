...
export async function readBLEGlucoseMeter(deviceManager: VitalDevicesManager) {
  let glucoseDeviceModel = VitalDevicesManager.supportedDevices.find(
    model =>
      model.kind === DeviceKind.GlucoseMeter && model.brand === Brand.AccuChek,
  );
  ...
  console.log(
    '@@@ Start scanning for glucose device: ' + glucoseDeviceModel.name,
  );
...
async function readScannedGlucoseMeter(
  ...
  console.log(
    '@@@ Start pairing device: ' + device.name + ' (id = ' + device.id + ')',
  );
...
export async function readBLEBloodPressureMeter(
  ...
  let bpDeviceModel = VitalDevicesManager.supportedDevices.find(
    model =>
      model.kind === DeviceKind.BloodPressure && model.brand === Brand.Omron,
  );

  if (bpDeviceModel === undefined) {
    throw Error(
      '@@@ [readBLEBloodPressureMeter] Cannot find the BLE device model.',
    );
  }
  ...
  console.log(
    '@@@ Start scanning for blood pressure device: ' + bpDeviceModel.name,
  );
  ...
  scanner = deviceManager.scanForDevice(bpDeviceModel, {
    onDiscovered: device => {
      console.log(
        '@@@ Discovered blood pressure meter: ' +
          device.name +
          ' (id = ' +
          device.id +
          ')',
...
async function checkBluetoothPermissions() {
  ...
  if (
    ...
    throw Error(
      '@@@ BLE permission not all granted; granted status = ' + statuses,
    );
  }
}
